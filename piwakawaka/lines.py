...
#     cursor = db.get_cursor()
#     lines = _fetch_lines_for_management(cursor)
#     assigned_lines = _fetch_lines_for_operator(cursor, session.get('user_id')) if session.get('role') == 'Operator' else []
#     cursor.close()

#     return render_template('lines/view_lines.html', lines=lines, assigned_lines=assigned_lines)
@app.route('/lines')
@login_required
def view_lines():
    cursor = db.get_cursor()
